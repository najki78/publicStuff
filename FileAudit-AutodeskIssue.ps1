...
# C:\Program Files\Common Files\Autodesk Shared\CLM\V5\MSVC14\cliccore\WebService.logconfig

# Reference: https://technochat.in/set-file-system-auditing-via-powershell/

# Prerequisite: enable Audit: Object Access
# https://www.wpninjas.ch/2020/01/configure-windows-10-auditing-with-intune/

$TargetFile = "C:\Program Files\Common Files\Autodesk Shared\CLM\V5\MSVC14\cliccore\WebService.logconfig"

# awesome script, thank you! - https://github.com/OTRF/Set-AuditRule/blob/master/Set-AuditRule.ps1
...
try {
    ...
    #remove existing audit rules - https://serverfault.com/questions/28925/how-to-remove-file-auditing-rules-via-powershell
    $acl = Get-Acl $TargetFile -Audit -ErrorAction Stop
    ...
    # WorldSid = Everyone ... https://docs.microsoft.com/en-us/dotnet/api/system.security.principal.wellknownsidtype?view=net-6.0
    Set-AuditRule -FilePath $TargetFile -WellKnownSidType WorldSid -Rights Delete -InheritanceFlags None -PropagationFlags None -AuditFlags Success,Failure

    #Get-Acl $TargetFile -Audit | fl
